...
*
     ...
     &                              info(3)
           ...
           write(ihist,'(A40)') '  Arnoldi b.e.    True b.e.    Fault'
         endif
*
       endif
...
*
       if ((irc(1).eq.0) .and. (irc(8) .eq. 0)) then
         icheck = 0
       endif
*
       return
...
* ---------------
      ...
      save dloo, nOrtho, compRsd, itotal
*
* Intrinsic function
* ------------------
...
*
        if (bn.eq.DZRO) then
          ...
          bea = DZRO
          be  = DZRO
          write(ihist,'(I5,11x,E8.2,$)') itotal,bea
...
*
        jH = 1
        if(itotal .gt. 1) itotal = itotal +1
 10     continue
...
* preconditioned system
       if ((bea.le.cntl(1)).or.(itotal.ge.iterMax).or.
     &      (irc(8).eq.1)) then  
* 
...
*
 ...
 43    continue
       if ((bea.le.cntl(1)).or.(itotal.ge.iterMax)) Then
...
* Save the backward error on a file if convergence history requested
         if (ihist.ne.0) then
           write(ihist,'(I5,11x,E8.2,$)') itotal,bea
           write(ihist,'(7x,E8.2,5x,I5)') be,irc(8)*2
         endif
*
       endif
*
...
* Check again the convergence
       if ((bea.le.cntl(1)).or.(itotal.ge.iterMax)) then
          if ((be.le.cntl(1)).or.(itotal.ge.iterMax)) then
             If(irc(8) .eq. 0) Then
...
*     Return the backward errors
                ...
                if (be.le.cntl(1)) then
                   ...
                   if (iwarn.ne.0) then
                      ...
                      write(iwarn,*) itotal,' outer iterations '
                      write(iwarn,*)
                   endif
                   if (ihist.ne.0) then
                      write(ihist,*)
                      ...
                      write(ihist,*) itotal,' outer iterations '
                      write(ihist,*)
                   endif
                   info(1) = -4
                endif
...
*     Save the backward error on a file if convergence history requested
          if (ihist.ne.0) then
             write(ihist,'(I5,11x,E8.2,$)') itotal,bea
             write(ihist,'(9x,A2,9x,I5)') '--',irc(8)*2
          endif
*     
       endif
*     Chek if there is fault, return
       if (irc(8) .eq. 1) then
          call dcopy(n,xCurrent,1,x,1)
          info(2) = itotal
          info(4) = jH
