...
* ---------------
       ...
       integer  iwarn, ihist
       integer  compRsd
       double precision    beta, bn,sA, sb, sPA, sPb, bea, be,temp,bea1
...
* ---------------
       ...
       save sA, sb, sPA, sPb, dnormx, trueNormRes, bea1, bea, be
       save dloo, nOrtho, compRsd
*
* Intrinsic function
* ------------------
       intrinsic dsqrt, abs, dreal
*
*
...
*
      if (bn.eq.DZRO) then
        ...
        bea = DZRO
        be  = DZRO
        write(ihist,'(I5,11x,E8.2,$)') jH,bea1
...
* Save the backward error on a file if convergence history requested
         if (ihist.ne.0) then
           write(ihist,1000)iterOut*m+jH,bea1,be
1000  format(I5,11x,E15.8,7x,E15.8)
         endif
*
       endif
*
